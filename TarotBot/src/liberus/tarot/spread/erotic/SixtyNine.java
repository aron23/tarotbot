...
public class SixtyNine extends Spread {
	...
	public SixtyNine(Interpretation myInt, String[] labels) {
		super(myInt);
		myNum = labels.length;
		myLabels = labels;
	}
	...
	public int getLayout() {
		return R.layout.sixtyninelayout;
	}

	public View populateSpread(View layout, AbstractTarotBotActivity act, Context ctx) {
		ImageView card = (ImageView) layout.findViewById(R.id.erotic_suck);
		...
		card = (ImageView) layout.findViewById(R.id.erotic_cream);
		...
		card = (ImageView) layout.findViewById(R.id.erotic_swallow);
		placeLandscapeImage(act.flipdex.get(2),card,ctx);
		card.setId(2);
		card.setOnClickListener(act);
		if (TarotBotActivity.secondSetIndex == 2)
			card.setColorFilter(0xFFFFFF00, PorterDuff.Mode.MULTIPLY);
		
		card = (ImageView) layout.findViewById(R.id.erotic_cum);
